...
module.exports = async function editTendaWithImg(conn, data, file, id, cb) {
  const validation = [];
  if (!data.nm_tenda) {
    validation.push({
  ...
  if (validation.length > 0) {
    ...
    await cekTenda(conn, id, (err, ada) => {
      if (err) {
        ...
        conn.query(
          'UPDATE tenda SET nm_tenda = ?, kapasitas = ?, ukuran = ?, bahan = ?, berat = ?, kelengkapan = ?, keterangan = ?, tarif = ?, stok = ?, durasi = ?, gambar = ? WHERE kd_tenda = ?',
          [
            data.nm_tenda,
            data.kapasitas,
            data.ukuran,
