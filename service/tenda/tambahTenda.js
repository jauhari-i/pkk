module.exports = async function tambahTenda(conn, data, file, cb) {
  const validation = [];
  if (!data.nm_tenda) {
    validation.push({
  ...
  if (validation.length > 0) {
    ...
    conn.query(
      'INSERT INTO tenda (nm_tenda,kapasitas,ukuran,bahan,berat,kelengkapan,keterangan,tarif,stok,durasi,gambar) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      [
        data.nm_tenda,
        data.kapasitas,
        data.ukuran,
