...
module.exports = userPinjam = async (conn, datas, cb) => {
  await conn.query(
    'SELECT * FROM pinjam INNER JOIN user ON pinjam.kd_user=user.kd_user INNER JOIN tenda ON pinjam.kd_tenda=tenda.kd_tenda WHERE pinjam.kd_user = ?',
    datas.kd_user,
    (err, data) => {
      if (err) {
        cb(err);
