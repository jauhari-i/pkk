...
module.exports = semuaPinjam = async (conn, cb) => {
  await conn.query(
    'SELECT * FROM pinjam INNER JOIN user ON pinjam.kd_user=user.kd_user INNER JOIN tenda ON pinjam.kd_tenda=tenda.kd_tenda',
    (err, data) => {
      if (err) {
        cb(err);
      } else {
