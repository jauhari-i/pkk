...
module.exports = satuPinjam = async (conn, datas, cb) => {
  await conn.query(
    'SELECT * FROM pinjam INNER JOIN user ON pinjam.kd_user=user.kd_user INNER JOIN tenda ON pinjam.kd_tenda=tenda.kd_tenda WHERE kd_pinjam = ?',
    datas.kd_pinjam,
    (err, data) => {
      if (err) {
        cb(err);
